...
import { updateUserProfile, uploadProfileImage } from "../../services/authService"; // Ensure these are implemented
...
const Profile: React.FC = () => {
  ...
  const [profileImageUrl, setProfileImageUrl] = useState<string | undefined>(user.profileImageUrl); // State for profile image URL
  ...
  return (
    <div className="flex relative z-10 w-[50rem] border-2">
      <div className="flex flex-col gap-10 w-[50%] items-center">
        ...
        <p className="text-lg">{displayName || "User"}<span>Display Name</span></p>
        <p>{address}</p>
      </div>

      <div className="bg-custom-gradient shadow-custom-shadow flex flex-col gap-10 w-[50%]">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleUpdateProfile}>
          ...
          <div className="form-group relative">
            <input
              ...
              accept="image/*"
              onChange={(e) =>
                setProfileImage(e.target.files ? e.target.files[0] : undefined)
              }
              disabled={isUpdating}
            />
            <FaUserEdit className="absolute right-2.5 top-2.5 text-purple-200 text-xl" />
...
// import { FaUserEdit } from "react-icons/fa";
// import { RiLockPasswordFill } from "react-icons/ri";
// import { selectUser } from "../../store/userSlice"; // Update with your actual path
// import {
//   updateUserProfile,
//   uploadProfileImage,
...
//   const [profileImage, setProfileImage] = useState<File | null>(null); // New state for profile image
...
//       // Upload profile image if selected
//       profileImageUrl = user.profileImageUrl; // Current URL or empty if not set
//       if (profileImage) {
